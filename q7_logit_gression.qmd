...
# 假设已有数据，包括实际值、预测值和地理位置  
# 示例数据  
data = pd.DataFrame({  
    'latitude': np.random.uniform(40.5, 41.0, 1000),  # 示例纬度  
    'longitude': np.random.uniform(-74.0, -73.5, 1000),  # 示例经度  
    'actual': np.random.randint(0, 2, 1000),  # 实际值  
    'predicted_probs': np.random.uniform(0, 1, 1000)  # 模型预测概率  
})  
...
# 构建空间权重矩阵（基于距离）  
# 这里我们设置一个距离阈值，比如 0.05 度（大约 5 公里），可以根据实际情况调整  
...
esda.plot_moran(moran, zstandard=True, ax=ax)  
