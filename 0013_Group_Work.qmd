...
title: NA_2 Group Project - Airbnb Trends and Patterns in London
assess:
...
import requests  
#import csl
url = "https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/harvard-cite-them-right.csl"  
...
#import bib
url = "https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/bio.bib"  
...
# loading data
file_path = 'https://raw.githubusercontent.com/VeraLi0710/FSDSGroup/refs/heads/main/data/listings_202312.csv'  
...
airbnb_data = airbnb_data[['host_id', 'room_type', 'availability_365', 'calculated_host_listings_count',  
                           'reviews_per_month', 'minimum_nights', 'estimated_nights_booked',  
                           'price', 'latitude', 'longitude','neighbourhood']].dropna()  
...
#import shp packages
# Link to the ZIP file in the GitHub repository (raw file link)  
file_path = "https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/data/statistical-gis-boundaries-london/ESRI/statistical-gis-boundaries-london.zip"  

# Step 1: Download the .zip file to a local file  
local_zip_path = "statistical-gis-boundaries-london.zip" 
...
# Step 2: Read the shapefile directly from the downloaded .zip file  
borough_map = gpd.read_file(f"zip://{os.path.abspath(local_zip_path)}")  
...
x_offset = 0.75  
y_offsets = [0.8, 0.6, 0.42, 0.22]  
subplot_width, subplot_height = 0.18, 0.12  

# -------------------------------  
# 3. Plot the Map of Airbnb Listings  
# -------------------------------  
...
# -------------------------------  
# 4. Subplot 1: Estimated Booking Days  
# -------------------------------  
...
airbnb_data['booking_color'] = airbnb_data['estimated_nights_booked'].apply(  
    lambda x: 'blue' if x <= 89 else 'red'  
)  

# Plot histogram  
sns.histplot(  
    data=airbnb_data, x='estimated_nights_booked', hue='booking_color',  
...
# -------------------------------  
# 6. Subplot 3: Room Type Count  
# -------------------------------  
ax3 = fig.add_axes([x_offset, y_offsets[2], subplot_width, subplot_height])  

# Plot bar chart  
room_type_count = airbnb_data['room_type'].value_counts()  
...
# -------------------------------  
# 7. Subplot 4: Minimum Nights Distribution  
# -------------------------------  
ax4 = fig.add_axes([x_offset, y_offsets[3], subplot_width, subplot_height])  

# Plot histogram  
sns.histplot(  
    airbnb_data['minimum_nights'], bins=1000, kde=False,  
...
file_urls = [  
    "https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/data/listings_20243.csv",  
    "https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/data/listings_20246.csv",  
    "https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/data/listings_20249.csv"  
]  

# Load the datasets directly from URLs  
data_list = [pd.read_csv(url) for url in file_urls]  

# Combine the datasets  
combined_data = pd.concat(data_list, ignore_index=True)  

# Combine with airbnb_data (2023 data)  
# Assuming you have already loaded `airbnb_data` elsewhere in your code  
...
merged_data = pd.concat([borough_counts, borough_counts_90], axis=1, keys=['Total_listings', 'More_than_90'])  

# Calculate the ratio of listings with more than 90 booked nights per total listings  
merged_data['Ratio_of_more_than_90'] = merged_data['More_than_90'] / merged_data['Total_listings']  

# Fill any NaN values that might occur if there are boroughs with no listings > 90 nights  
merged_data['Ratio_of_more_than_90'] = merged_data['Ratio_of_more_than_90'].fillna(0)  

# Data formatting and round to four decimal places  
merged_data['Ratio_of_more_than_90'] = merged_data['Ratio_of_more_than_90'].apply(lambda x: round(x, 4))  

# Rename the index label to 'Borough_name'  
merged_data.index.rename('Borough_name', inplace=True)  

# Load the borough codes  
borough_code_file_path = 'https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/data/borough_name_code.csv'  
borough_codes = pd.read_csv(borough_code_file_path)  

# Reset index in merged_data to turn the index into a regular column  
merged_data.reset_index(inplace=True)  
borough_codes.reset_index(inplace=True)  

# Combine the ratio data and borough name with borough code by borough name  
merged_data = pd.merge(merged_data, borough_codes[['Borough_name', 'Borough_code']], on='Borough_name', how='left')  

# Set 'Borough_name' back as the index  
merged_data.set_index('Borough_name', inplace=True)  

# Save the updated DataFrame  
merged_data.to_csv('data/borough_listings_ratio.csv', index=True)
```
...
# load data
connect = pd.read_csv("https://github.com/VeraLi0710/FSDSGroup/raw/refs/heads/main/data/connect.csv")
...
for i, var in enumerate(var_names):
    ...
    threshold = 1.96
    non_significant = zone['t_values'].abs() < threshold  
...
# Path of png 
img = mpimg.imread('plots/Results_of_SAR_and_GWR_model.png')  
...
listing = pd.read_csv(
    "https://raw.githubusercontent.com/VeraLi0710/FSDSGroup/main/data/processed_airbnb_data.csv"
...
## 2.2. Load London ward boundaries
# Shapefiles
files = [
...
# GitHub pathway
base_url = "https://raw.githubusercontent.com/VeraLi0710/FSDSGroup/main/data/statistical-gis-boundaries-london/ESRI/"
output_dir = "./shapefile"  # save to local
...
# read Shapefile and convert crs
shapefile_path = os.path.join(output_dir, "London_Ward_CityMerged.shp")
ward = gpd.read_file(shapefile_path).to_crs(epsg=27700)
...
# GitHub pathway
base_url = "https://raw.githubusercontent.com/VeraLi0710/FSDSGroup/main/data/ldn_poi/"
output_dir = "./ldn_poi"  # save to local
os.makedirs(output_dir, exist_ok=True)
...
# read Shapefile and convert crs
shapefile_path = os.path.join(output_dir, "gis_osm_natural_free_1.shp")
nature = gpd.read_file(shapefile_path).to_crs(epsg=27700)
...
# read Shapefile and convert crs
shapefile_path = os.path.join(output_dir, "gis_osm_transport_free_1.shp")
transport = gpd.read_file(shapefile_path).to_crs(epsg=27700)
...
# read Shapefile and convert crs
shapefile_path = os.path.join(output_dir, "gis_osm_pois_free_1.shp")
tourism = gpd.read_file(shapefile_path).to_crs(epsg=27700)
...
# Step 6: Spatial Join and Filtering Airbnb Data
## 6.1. Perform a spatial join: Airbnb data with ward boundaries
listing_ward = gpd.sjoin(listing_spatial, ward, how="left", predicate="intersects")

## 6.2. Select relevant columns to keep
columns_to_keep = [
    "id",
    "room_type",
    "price",
...
# Step 7: Calculate Average Price and Minimum Nights per Ward
# --------------------------------------------------------------
## 7.1 Calculate Average Price
### 7.1.1 Group by GSS_CODE and calculate the average price
avg_price = listing_ward.groupby("GSS_CODE", as_index=False)["price"].mean()
avg_price.rename(columns={"price": "avg_price"}, inplace=True)

### 7.1.2. Merge the average price back to the ward data
ward["avg_price"] = ward["GSS_CODE"].map(avg_price.set_index("GSS_CODE")["avg_price"])

### 7.1.3. Check the updated ward data
print(ward.info())

# --------------------------------------------------------------
## 7.2 Calculate Average Minimum Nights
...
### 7.2.2 Merge the average minimum nights back to the ward data
ward["avg_night"] = ward["GSS_CODE"].map(avg_night.set_index("GSS_CODE")["avg_night"])

### 7.2.3 Check the updated ward data
print(ward.info())
```
...
# Step 8: Calculate Average Host Listings and Entire Home Density
# --------------------------------------------------------------
## 8.1 Average Host Listings
### 8.1.1 Calculate the average number of host listings per ward
...
### 8.1.2. Merge the average host listings back to the ward data
...
# --------------------------------------------------------------
## 8.2 Calculate 'Entire Home' Density
### 8.2.1 Filter rows where room_type contains 'Entire'
...
### 8.2.2 Merge the count of 'Entire' listings back to the ward data
ward = ward.merge(entire_count, on="GSS_CODE", how="left")

### 8.2.3 Calculate the density of 'Entire' listings per ward area
ward["entire_density"] = ward["entire_count"] / ward["area"]

### 8.2.4 Fill NaN values for wards with no 'Entire' listings
ward["entire_count"] = ward["entire_count"].fillna(0)
ward["entire_density"] = ward["entire_density"].fillna(0)

### 8.2.5. Check the updated ward data
print(ward.info())
```
...
# Step 9: Save Results to Shapefile
## 9.1 Define the columns to retain
columns_to_keep = [
    "listing_density",
    "nature_density",
    "transport_density",
...
## 9.2 Create a new dataset with selected columns and save as Shapefile
...
# Step 10: Perform GWR
# --------------------------------------------------------------
## 10.1 Prepare GWR Input Data
### 10.1.1 Read the Shapefile data for GWR
gwr = gwr_data

### 10.1.2 Extract centroids and coordinates for GWR
gwr["geometry"] = gwr.centroid
gwr["x"] = gwr.geometry.x
gwr["y"] = gwr.geometry.y

### 10.1.3 Define dependent variable (Y), independent variables (X), and coordinates
Y = gwr[["listing_density"]].values
X = gwr[
    [
        "nature_density",
        "transport_density",
...
if os.path.exists(cache_file):
    ...
    ### 10.2.1 Determine the optimal bandwidth
    bw = Sel_BW(coords, Y, X).search()
    
    ### 10.2.2 Fit the GWR model
    model = GWR(coords, Y, X, bw)
    results = model.fit()
    
    ### 10.2.3 Save to cache
...
### 10.2.4 Print summary of GWR results
...
# --------------------------------------------------------------
## 10.3 Extract Local R² and Coefficients
...
## 11.1. Set up the figure with a 2x4 layout
fig, axes = plt.subplots(2, 4, figsize=(16, 8))  # 2 rows, 4 columns

## 11.2. Define the variables and custom titles for each subplot
variables = [
...
## 11.3. Loop through the variables and plot each on the corresponding axis
...
## 11.4. Adjust layout and display the figure
