...
```{python}
# VIF and check for highly correlated variables
```
...
# Readable visualisation for correlation matrix
...
# Assuming `result` is the fitted logistic regression model  
# Predicted probabilities (fitted values) and residuals  
fitted_values = result.predict()  
residuals = Y - fitted_values  

# 查看预测值（fitted_values）范围  
print(f"Fitted values range: {fitted_values.min()} to {fitted_values.max()}")  

# 查看残差的基本统计情况  
print(f"Residuals range: {residuals.min()} to {residuals.max()}")  

# 检测是否存在极端的标准化残差值  
extreme_std_residuals = standardized_residuals[np.abs(standardized_residuals) > 3]  
print(f"Number of extreme standardized residuals (>3): {len(extreme_std_residuals)}")
...
# 2. Scale-Location Plot (Square Root of Standardized Residuals vs Fitted Values)  
standardized_residuals = residuals / np.std(residuals)  
sqrt_standardized_residuals = np.sqrt(np.abs(standardized_residuals))  

plt.figure(figsize=(10, 6))  
plt.scatter(fitted_values, sqrt_standardized_residuals, alpha=0.7, edgecolor='k')  
plt.axhline(0, color='red', linestyle='--', linewidth=1)  
plt.xlabel('Fitted Values')  
...
# Extract leverage values from the model  
...
plt.figure(figsize=(10, 6))  
plt.scatter(leverage, standardized_residuals, alpha=0.7, edgecolor='k')  
plt.axhline(0, color='red', linestyle='--', linewidth=1)  
plt.xlabel('Leverage')  
plt.ylabel('Standardized Residuals')  
...
threshold = 4 / len(fitted_values)  # Common threshold for Cook's distance  
plt.scatter(leverage, standardized_residuals, c=cooks_d, cmap='coolwarm', edgecolor='k')  
plt.colorbar(label="Cook's Distance")  
plt.axhline(0, color='red', linestyle='--', linewidth=1)  
plt.axvline(threshold, color='blue', linestyle='--', linewidth=1, label="Cook's Distance Threshold")  
plt.legend()  
plt.show()  

# use statsmodels' built-in leverage-residual plot  
