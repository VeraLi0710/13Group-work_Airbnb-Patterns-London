...
1. The data comes solely from Airbnb, which may delete or hide listings due to business or regulatory pressures, reducing representativeness. For example, in 2015, Airbnb removed over 1,000 entire-home listings in New York City [@cox_how_2016].
2. The data's snapshot nature cannot reflect ongoing changes, such as newly added or removed listings. This compromises its temporal completeness. Meanwhile, when new versions of dataset overwrite old ones, there are significant differences in entries and reviews across releases. Without specifying the dataset version, research results may lack reproducibility and reliability.
...
4. The anonymization of geolocation data shifts locations by up to 150 meters and scatters listings within buildings, reducing spatial accuracy for neighborhood analysis.
5. Airbnb's 'Experience' and 'Listing' features sometimes unintentionally have the same ID, despite representing different entities. This leads to 'Experience' reviews being misclassified as 'Listing' reviews, causing inaccuracies in review data [@alsudais_incorrect_2021].
:::

## 5. What ethical considerations does the use of the InsideAirbnb data raise? 

::: 
1. **Respecting the Inside Airbnb's Vision: Ensuring Data Serves the Public Good**  
Inside Airbnb’s core value is to reveal the impacts of short-term rentals and advocate for local communities. If used for some commercial purposes, the data could harm residents. For instance, investors might use the data to identify high-profit areas for short-term rentals, leading to increased property purchases for rentals and worsening the housing crisis. Therefore, researchers and policymakers should align their work with the mission by prioritizing community interests and exercising caution when sharing data with commercial entities to prevent misuse that could harm local communities.
3. **Protecting User Privacy**  
Data collection relies on web scraping technology, meaning Airbnb hosts and guests may not have explicitly authorized the use of their data. Meanwhile, even when geographic data is anonymized, it can unintentionally reveal location details in small communities or standalone properties. Short-term rental activity is sometimes associated with housing shortages, rising rents, and negative impacts on local residents. Once short-term rental listings are identified, hosts and guests may face risks such as harassment, discrimination, or economic loss. Therefore, research outputs should be generalized to prevent the indirect identification of specific properties and avoid language that reinforces negative perceptions of individuals.
5. **Ensuring Representation of Vulnerable Communities: Promoting Equity in Data Interpretation**  
Although Inside Airbnb data is open, ordinary residents, especially those in low-income communities, often lack the technical skills to analyze and interpret the data. As a result, the power to interpret the data is concentrated in the hands of a few technically skilled residents, activists, and researchers. This may lead to policymaking that favors the needs of these elite groups, while potential information related to vulnerable communities in the data may be underestimated, overlooked, or misunderstood. Data-driven policies and decisions should not serve only as tools for technical elites but should act as a means to promote social fairness and justice. Therefore, as discussed on ‘Data Feminism’, researchers and policymakers should consciously consider vulnerable communities, encourage their participation in data analysis and interpretation, and present findings in an accessible way to amplify their voices [@dignazio_data_2020]
:::
...
1. **Central London is a Compliance Hotspot**:  
    Central London shows the highest concentration of short-term rentals, with entire homes dominating in high-demand areas prone to violations of the 90-day rule (Figure 1, Map).
...
3. **Multi-Listing Hosts Dominate the Market**:  
    Multi-listing hosts manage 62.38% of Airbnb listings with longer average booking durations than single-listing hosts. They primarily operate entire homes and hotel-like properties, which are highly commercialized with shorter stays and higher prices (Figure 2, Top Left; heatmap).

4. **Entire Homes are the Highest Risk**:  
    Entire homes make up 66.97% of listings, with an highest average booking days, and are most likely to breach the 90-day rule. Hotel-like properties, while fewer, also show risks due to short minimum stays (2.13 nights) and high commercialization (Figure 2, Top Right; heatmap).




### Summary and Next Steps  
InsideAirbnb data shows London’s Airbnb market is highly commercialized, with significant 90-day rule compliance risks, requiring effective enforcement.

1. **Spatial Insights**: Airbnb listings are clustered in high-risk areas - further spatial analysis is needed to assess impacts and identify drivers of non-compliance for better regulation.

2. **Policy Violation Factors**: Multi-listing hosts and commercialized properties, such as entire homes and hotel-like listings, are major contributors to non-compliance. Different factors (e.g. property type, cost, location...)need further regression analysis to improve enforcement.
...
# Calculate Residuals Using Existing Predicted Probabilities  
regression_data["residuals"] = regression_data["estimated_nights_booked_binary"] - regression_data["predicted_probs"]  
...
print(regression_data["residuals"].describe())  

# Display Deviance Residuals Summary Statistics  
print("\nDeviance Residuals Summary Statistics:")  
print(regression_data["deviance_residuals"].describe())  

# Display Extreme Deviance Residuals  
print("\nExtreme Deviance Residuals (Top 5 Largest Absolute Values):")  
extreme_residuals = regression_data["deviance_residuals"].abs().sort_values(ascending=False).head()  
print(extreme_residuals)  

# Display Predicted Probabilities Summary Statistics  
print("\nPredicted Probabilities Summary Statistics:")  
print(regression_data["predicted_probs"].describe())  

# Check Distribution of Actual vs Predicted  
print("\nActual vs Predicted Cross-tabulation:")  
print(pd.crosstab(  
    regression_data["estimated_nights_booked_binary"],  
    (regression_data["predicted_probs"] > 0.5).astype(int),  
    margins=True  
))  
...
Additionally, residual analysis and Moran's I test (I = 0.0855, p = 0.001) highlighted weak but significant spatial patterns, indicating missing factors and spatial heterogeneity. Further model improvements are needed to better capture these complexities.
